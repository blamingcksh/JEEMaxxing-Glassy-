/* forest-island.js — Daily Grove (complete, self-contained, persistent) */
(function () {
'use strict';
...
function getStoredToday(){var s=loadStore();var c=s[todayKey()]||{};return {physics:(+c.physics||0),chemistry:(+c.chemistry||0),maths:(+c.maths||0)};}
function saveStoredToday(c){var s=loadStore();s[todayKey()]={physics:c.physics||0,chemistry:c.chemistry||0,maths:c.maths||0,updatedAt:Date.now()};saveStore(s);}
function readLiveCounts(){function g(id){var e=document.getElementById(id);return e?(parseInt(e.textContent,10)||0):0;}var l={physics:g('physics-count'),chemistry:g('chemistry-count'),maths:g('maths-count')};try{if(window.solved){l.physics=Math.max(l.physics,(+window.solved.physics||0));l.chemistry=Math.max(l.chemistry,(+window.solved.chemistry||0));l.maths=Math.max(l.maths,(+window.solved.maths||0));}}catch(e){}return l;}
function readVisualCounts(){var l=readLiveCounts();var s=getStoredToday();return {physics:Math.max(l.physics,s.physics),chemistry:Math.max(l.chemistry,s.chemistry),maths:Math.max(l.maths,s.maths)};}
...
function rebuildTerrain(){
  ...
  var S=LAND_R*1.5;var seg=Math.min(140,Math.max(96,Math.round(LAND_R*9)));
  var lg=new THREE.PlaneGeometry(S*2,S*2,seg,seg);lg.rotateX(-Math.PI/2);
  var lp=lg.attributes.position;var lc=new Float32Array(lp.count*3);
  for(var i=0;i<lp.count;i++){var x=lp.getX(i),z=lp.getZ(i),h=iHeight(x,z);lp.setY(i,h);var r=Math.hypot(x,z);var th=Math.atan2(z,x);var t=Math.min(1,r/coastR(th));if(h<-0.1){lc[i*3]=0.36;lc[i*3+1]=0.28;lc[i*3+2]=0.16;}else if(t>0.78){lc[i*3]=0.74;lc[i*3+1]=0.62;lc[i*3+2]=0.34;}else if(h>1.25){lc[i*3]=0.42;lc[i*3+1]=0.46;lc[i*3+2]=0.40;}else{var g=0.44+0.34*(1-t);lc[i*3]=0.08+0.08*t;lc[i*3+1]=g;lc[i*3+2]=0.10;}}
...
function allocSpot(minDist){var ds=iEnv&&iEnv.drySpots;if(!ds||!ds.length)return null;minDist=minDist||1.85;var trees=allPlacedTrees();var cell=Math.max(1.25,minDist);var grid={};function gkey(x,z){return Math.floor(x/cell)+','+Math.floor(z/cell);}for(var i=0;i<trees.length;i++){var t=trees[i];var key=gkey(t.x,t.z);if(!grid[key])grid[key]=[];grid[key].push(t);}function tooClose(x,z){var cx=Math.floor(x/cell);var cz=Math.floor(z/cell);var md2=minDist*minDist;for(var dx=-1;dx<=1;dx++)for(var dz=-1;dz<=1;dz++){var arr=grid[(cx+dx)+','+(cz+dz)];if(!arr)continue;for(var j=0;j<arr.length;j++){var ddx=arr[j].x-x;var ddz=arr[j].z-z;if(ddx*ddx+ddz*ddz<md2)return true;}}return false;}var start=(Math.random()*Math.min(64,ds.length))|0;for(var k=0;k<ds.length;k++){var idx=(start+k)%ds.length;var s=ds[idx];var x=s.x+(Math.random()-0.5)*0.7;var z=s.z+(Math.random()-0.5)*0.7;var y=iHeight(x,z);if(y<0.28)continue;if(tooClose(x,z))continue;return {idx:idx,x:x,y:y,z:z};}return null;}
...
function addIsland(q,instant){if(!iBuilt)return;var oak=(q.qElo||1200)>=2300,k=oak?'oak':q.subject;var minD=oak?2.7:1.85;var sp=allocSpot(minD);if(!sp){expandIsland();sp=allocSpot(minD)||allocSpot(minD*0.82);}if(!sp){var fb=(iEnv.drySpots&&iEnv.drySpots.length)?iEnv.drySpots[(Math.random()*iEnv.drySpots.length)|0]:{x:0,y:0.5,z:0};sp={idx:-1,x:fb.x+(Math.random()-0.5)*0.8,y:Math.max(0.28,iHeight(fb.x,fb.z)),z:fb.z+(Math.random()-0.5)*0.8};}var base=(0.95+Math.min(1,Math.max(0,((q.qElo||1200)-800)/2200))*0.95);var s={x:sp.x,y:sp.y,z:sp.z,subject:k,qElo:q.qElo||1200,oak:oak,plantedAt:Date.now(),baseScale:base,sy:0.9+hash(iTotal(),11)*0.4,sxz:0.95+hash(iTotal(),13)*0.3,leanX:(hash(iTotal(),17)-0.5)*0.08,leanZ:(hash(iTotal(),19)-0.5)*0.08,rot:hash(iTotal(),3)*6.283,cur:instant?1:0,animT0:performance.now(),iid:iState[k].length};iState[k].push(s);writeIsland(k,s,s.cur);iMeshes[k].count=iState[k].length;iMeshes[k].instanceMatrix.needsUpdate=true;if(window.__forestIslandAPI){var interactive=!instant&&!window.__forestIslandAPI.initialSync;for(var h=0;h<window.__forestIslandAPI.onPlanted.length;h++){try{window.__forestIslandAPI.onPlanted[h](s,interactive);}catch(e){}}}}
...
function syncToLive(){if(!iBuilt)return;var live=readVisualCounts(),today=todayBySubject();var totalWant=(live.physics||0)+(live.chemistry||0)+(live.maths||0);while(iBuilt&&landCapacity()<totalWant&&LAND_R<MAX_LAND_R)expandIsland();SUBJ.forEach(function(subj){var want=live[subj]||0,have=plantedBySubj[subj];while(have<want){var q=(today[subj]&&today[subj][have])||{subject:subj,qElo:1200};addIsland(q,false);have++;plantedBySubj[subj]=have;}if(have>want){var drop=have-want;for(var d=0;d<drop;d++)iState[subj].pop();plantedBySubj[subj]=want;if(iMeshes[subj]){iMeshes[subj].count=iState[subj].length;iMeshes[subj].instanceMatrix.needsUpdate=true;}}});setCount(iTotal());}
function iframe(t){if(!iBuilt||!iVisible||document.hidden){iRaf=null;return;}iRaf=requestAnimationFrame(iframe);if(t-iLast<IFRAME)return;var dt=Math.min(0.05,(t-iLastT)/1000||0);iLastT=t;iLast=t;iEl+=dt;if(motionOK())iOrbit+=dt*0.12;var camD=9.4+Math.max(0,LAND_R-MIN_LAND_R)*0.55;iCam.position.set(Math.sin(iOrbit)*camD,6.2+Math.max(0,LAND_R-MIN_LAND_R)*0.18,Math.cos(iOrbit)*camD);iCam.lookAt(0,1.25,0);if(t-iLastTOD>30000){iLastTOD=t;iApplyTOD(realTOD());}if(iEnv.water)iEnv.water.position.y=-0.2+Math.sin(iEl*0.8)*0.02;if(itreeMat.userData.shader)itreeMat.userData.shader.uniforms.uTime.value=iEl;var now=performance.now();for(var k in iState){var arr=iState[k],dirty=false;for(var i=0;i<arr.length;i++){var s=arr[i];if(s.cur<1){var p=(now-s.animT0)/600,g;if(p<=0)g=0;else if(p>=1){g=1;s.cur=1;}else{g=easeOutBack(p);s.cur=g;}writeIsland(k,s,g);dirty=true;}}if(dirty)iMeshes[k].instanceMatrix.needsUpdate=true;}if(window.__forestIslandAPI)for(var h=0;h<window.__forestIslandAPI.onFrame.length;h++){try{window.__forestIslandAPI.onFrame[h](iEl,dt);}catch(e){}}iRenderer.render(iScene,iCam);}
...
/* single router → embedded full island (no more iframe / forest-lab.html) */
...
function mount(){
  ...
  /* ── smart counter observer: persistence + self-heal, no double-plant ── */
  var manualTick=0;
  document.addEventListener('pointerdown',function(e){try{if(e.target&&e.target.closest&&e.target.closest('.counter-btn'))manualTick=Date.now();}catch(_){}},true);
  var lastLive=readLiveCounts();
  var suppress=false;
  function setSubj(subj,v){var node=document.getElementById(subj+'-count');if(node&&(parseInt(node.textContent,10)||0)!==v)node.textContent=String(v);try{if(window.solved)window.solved[subj]=v;}catch(_){}}
  function restoreAssert(){var st=getStoredToday();var live=readLiveCounts();var changed=false;SUBJ.forEach(function(s){var want=st[s]||0;if(want>(live[s]||0)){setSubj(s,want);changed=true;}});if(changed)lastLive=readLiveCounts();}
  function seedStore(){var st=getStoredToday();var live=readLiveCounts();SUBJ.forEach(function(s){st[s]=Math.max(st[s]||0,live[s]||0);});saveStoredToday(st);lastLive=readLiveCounts();}
  function onCountersMut(){if(suppress)return;var live=readLiveCounts();var now=Date.now();var manual=(now-manualTick)<300;var st=getStoredToday();var write=false;SUBJ.forEach(function(s){var d=(live[s]||0)-(lastLive[s]||0);if(manual){st[s]=live[s]||0;write=true;}else if(d>0){if((live[s]||0)>(st[s]||0)){st[s]=live[s]||0;write=true;}}else if(d<0){if((st[s]||0)>(live[s]||0)){suppress=true;setSubj(s,st[s]||0);suppress=false;}}});if(write)saveStoredToday(st);lastLive=readLiveCounts();}
  try{counterObs=new MutationObserver(function(){requestAnimationFrame(function(){onCountersMut();syncToLive();if(iVisible&&!iBuilt)ensureIslandBuilt();});});['physics-count','chemistry-count','maths-count'].forEach(function(id){var e=document.getElementById(id);if(e)counterObs.observe(e,{childList:true,subtree:true,characterData:true});});}catch(e){}
  restoreAssert();seedStore();syncToLive();
